...
use mqtt_format::v3::{
    qos::MQualityOfService, strings::MString, subscription_request::MSubscriptionRequest,
    will::MLastWill,
};
use tracing::error;
...
async fn main() {
    ...
    match args.subcommand {
        Subcommand::Subscribe { subscriptions } => {
            client
                .subscribe(
                    &subscriptions
                        .iter()
        ...
        Subcommand::Publish { topic, message } => {
            match client
                .publish(
